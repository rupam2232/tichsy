...
import { useId, useRef, useEffect, type HTMLAttributes } from "react";
...
export function Phone({
  ...
  const id = useId();
  const maskId = `screenPunch-${id.replace(/:/g, "")}`;
  const clipId = `roundedCorners-${id.replace(/:/g, "")}`;
  const videoRef = useRef<HTMLVideoElement>(null);

  useEffect(() => {
    if (!videoRef.current) return;
    if (playing) {
      // Add a small delay to ensure video is ready and browser can process the play request
      const playPromise = videoRef.current.play();
      if (playPromise !== undefined) {
        playPromise.catch(() => {
          // Play was interrupted or not allowed, ignore gracefully
        });
      }
    } else {
  ...
  return (
    ...
    >
      ...
      {!hasMedia && props.children && (
        <div
          className="absolute z-1 overflow-hidden bg-background"
          style={{
            left: `${LEFT_PCT}%`,
            top: `${TOP_PCT}%`,
            width: `${WIDTH_PCT}%`,
